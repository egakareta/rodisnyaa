...
fn main() -> Result<(), rodisnyaa::NyaaError> {
    ...
    let mut list = false;
    let mut backend_name = Output::DEFAULT_BACKEND_LABEL.to_string();
    ...
    if list {
        for label in Output::available_backend_labels() {
            println!("{label}");
        }
        return Ok(());
    }
    ...
    if !nyaa.set_preferred_backend_by_name(&backend_name) {
