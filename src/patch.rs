...
/// Installs browser compatibility shims into every `AudioWorklet` before CPAL loads its worklet
/// module.
///
...
///
/// It also lacks the `self` alias and worker-global event methods expected by
/// wasm-bindgen-rayon's statically imported worker helper. In the worklet realm those listeners
/// are inert; real Rayon workers retain their native globals.
///
